...
- The capacity to make a Select/Update/Insert for each row to save with `Paillave.Etl.EntityFrameworkCore` or `Paillave.Etl.SqlServer`

The principle is to be able, for each payload issued in any operator, to know what was the initial row or rows that permitted to result to a payload.

Behind the scenes, each row has a list of unique identifiers linked to it, and each operator is supposed to know how to combine this these list of uid regarding to its purpose. When a stream is issuing payloads that have a list unique identified under the hood is called a **correlated stream**.
...
Then, let's this stream based on the business key (`Value2`). As the save system gets the Id wether an occurrence already exists in the database or not, we get this result:
...
It can happen sometimes that, depending on the situation, C# compiler cannot resolve properly which version of `EfCoreSave` to use and take the one for non correlated streams. If so, use `EfCoreSaveCorrelated` instead to prevent the compiler to be confused.
