...
namespace EFCore.BulkExtensions
{
    public class TableInfo
    {
        ...
        //public bool HasSinglePivotKey { get; set; }
        ...
        public bool CreatedOutputTable => (BulkConfig.SetOutputIdentity && HasIdentityColumn) || BulkConfig.CalculateStats;

        public bool InsertToTempTable { get; set; }
        //public bool IsPivotIdentity { get; set; }
        ...
        public void LoadData<T>(DbContext context, bool loadOnlyPKColumn)
        {
            ...
            //HasSinglePivotKey = PivotKeys.Count == 1;
            ...
            if (AreSpecifiedPropertiesToInclude)
            {
                ...
                {
                    foreach (var primaryKey in PivotKeys)
                    {
            ...
            if (loadOnlyPKColumn)
            {
                PropertyColumnNamesDict = properties.Where(a => PivotKeys.Contains(a.Name)).ToDictionary(a => a.Name, b => b.Relational().ColumnName);
            }
            else
            {
        ...
        public Dictionary<string, string> ConfigureBulkReadTableInfo(DbContext context)
        {
            ...
            BulkConfig.PropertiesToInclude = PivotKeys;
            PropertyColumnNamesDict = PropertyColumnNamesDict.Where(a => PivotKeys.Contains(a.Key)).ToDictionary(i => i.Key, i => i.Value);
        ...
        public void UpdateReadEntities<T>(IList<T> entities, IList<T> existingEntities)
        {
            ...
            List<string> selectByPropertyNames = PropertyColumnNamesDict.Keys.Where(a => PivotKeys.Contains(a)).ToList();
        ...
        public void UpdateEntitiesIdentity<T>(IList<T> entities, IList<T> entitiesWithOutputIdentity)
        {
        ...
        public IEnumerable<T> ExecuteQuery<T>(DbContext context, string sqlQuery) where T : class
        {
            if (BulkConfig.TrackingEntities) // If Else can not be replaced with Ternary operator for Expression
            {
                return context.Set<T>().IgnoreQueryFilters().FromSql(sqlQuery);
            }
            else
            {
                return context.Set<T>().IgnoreQueryFilters().FromSql(sqlQuery).AsNoTracking();
            }
        }
        ...
        public Expression<Func<DbContext, IQueryable<T>>> GetQueryExpression<T>(string sqlQuery) where T : class
        {
            ...
            var ordered = OrderBy(expression, PivotKeys[0]);
