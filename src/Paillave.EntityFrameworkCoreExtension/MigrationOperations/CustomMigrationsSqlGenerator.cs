...
{
    private readonly Assembly _assembly = dependencies.CurrentContext.Context.GetType().Assembly;
    ...
    {
        Regex regex = new(@"[\s\r]*create([\s\r]+or[\s\r]+alter)?([\s\r]+clustered)?([\s\r]+unique)?[\s\r]+(?<type>\w+)[\s\r]+(?<name>[^\s\r]+)[\s\r]+(?<body>.*)", RegexOptions.Singleline | RegexOptions.IgnoreCase);
