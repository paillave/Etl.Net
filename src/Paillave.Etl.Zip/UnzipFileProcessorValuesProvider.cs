...
public class UnzipFileProcessorValuesProvider(UnzipFileProcessorParams args) : ValuesProviderBase<IFileValue, IFileValue>
{
    ...
    public void PushValues(IFileValue input, Action<IFileValue> push, CancellationToken cancellationToken)
    {
        ...
        using var stream = input.Get(args.UseStreamCopy);
        using var zf = new ZipFile(stream);
        var searchPattern = string.IsNullOrEmpty(args.FileNamePattern) ? "*" : args.FileNamePattern;
        var matcher = new Matcher().AddInclude(searchPattern);

        if (!String.IsNullOrEmpty(args.Password))
            zf.Password = args.Password;
