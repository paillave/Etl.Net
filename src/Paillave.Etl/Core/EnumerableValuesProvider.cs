...
{
    public static EnumerableValuesProvider<TIn, TOut> Create<TIn, TOut>(Func<TIn, IEnumerable<TOut>> getValues) => new(getValues);
    public static EnumerableValuesProvider<TIn1, TIn2, TOut> Create<TIn1, TIn2, TOut>(Func<TIn1, TIn2, IEnumerable<TOut>> getValues) => new(getValues);
}
public class EnumerableValuesProvider<TIn, TOut>(Func<TIn, IEnumerable<TOut>> getValues) : ValuesProviderBase<TIn, TOut>
{
    private readonly Func<TIn, IEnumerable<TOut>> _getValues = getValues;
...
{
    private readonly Func<TIn1, TIn2, IEnumerable<TOut>> _getValues = getValues;
