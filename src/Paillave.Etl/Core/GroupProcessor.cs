...
{
    public static GroupProcessor<TIn, TMultiKey> Create<TIn, TMultiKey>(Func<TIn, TMultiKey> getKeys) => new(getKeys);
}
public class GroupProcessor<TIn, TMultiKey>(Func<TIn, TMultiKey> getKeys)
{
    private readonly Func<TIn, TMultiKey> _getKeys = getKeys;
    private static readonly PropertyInfo[] _keyProperties = typeof(TMultiKey).GetProperties().ToArray();
    private readonly Dictionary<(string PropertyName, object Value), (ObjectBuilder<TMultiKey> key, List<TIn>)> _keysDictionaries = new();
    private readonly List<TIn> _rowsWithNoKey = new();
