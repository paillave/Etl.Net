...
public class SubstractSubject<TLeft, TRight, TKey> : PushSubject<TLeft>
{
    ...
    private class SubscriptionItem<T>(IPushObservable<T> observable, Action<T> onPushValue, Action onComplete, Action<Exception> onException) : IDisposable
    {
        ...
        private readonly IDisposable _subscription = observable.Subscribe(onPushValue, onComplete, onException);
        private readonly IPushObservable<T> _observable = observable;
