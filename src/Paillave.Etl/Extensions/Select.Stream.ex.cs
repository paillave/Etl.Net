...
public static partial class SelectEx
{
    ...
    public static IStream<TOut> Select<TIn, TOut>(this IStream<TIn> stream, string name, Func<TIn, TOut> resultSelector, bool withNoDispose = false) =>
        new SelectStreamNode<TIn, TOut>(name, new SelectArgs<TIn, TOut>
        {
            Stream = stream,
    ...
    public static ISingleStream<TOut> Select<TIn, TOut>(this ISingleStream<TIn> stream, string name, Func<TIn, TOut> resultSelector, bool withNoDispose = false) =>
        new SelectSingleStreamNode<TIn, TOut>(name, new SelectSingleArgs<TIn, TOut>
        {
            Stream = stream,
            Processor = new SimpleSelectProcessor<TIn, TOut>(resultSelector),
            WithNoDispose = withNoDispose
    ...
    public static IStream<TOut> Select<TIn, TOut>(this IStream<TIn> stream, string name, ISelectWithIndexProcessor<TIn, TOut> processor, bool withNoDispose = false) =>
        new SelectWithIndexStreamNode<TIn, TOut>(name, new SelectWithIndexArgs<TIn, TOut>
        {
            Stream = stream,
            Processor = processor,
            WithNoDispose = withNoDispose
    ...
    public static IStream<TOut> Select<TIn1, TIn2, TOut>(this IStream<TIn1> stream, string name, ISingleStream<TIn2> streamToApply, Func<TIn1, TIn2, int, TOut> resultSelector, bool withNoDispose = false) =>
        new ApplyStreamNode<TIn1, TIn2, TOut>(name, new ApplyArgs<TIn1, TIn2, TOut>
        {
            MainStream = stream,
    ...
    public static ISingleStream<TOut> Select<TIn1, TIn2, TOut>(this ISingleStream<TIn1> stream, string name, ISingleStream<TIn2> streamToApply, Func<TIn1, TIn2, int, TOut> resultSelector, bool withNoDispose = false) =>
        new ApplySingleStreamNode<TIn1, TIn2, TOut>(name, new ApplySingleArgs<TIn1, TIn2, TOut>
        {
            MainStream = stream,
            StreamToApply = streamToApply,
            IndexSelector = resultSelector,
    ...
    public static IStream<Correlated<TOut>> Select<TIn, TOut>(this IStream<Correlated<TIn>> stream, string name, Func<TIn, TOut> resultSelector, bool withNoDispose = false) =>
        new SelectStreamNode<Correlated<TIn>, Correlated<TOut>>(name, new SelectArgs<Correlated<TIn>, Correlated<TOut>>
        {
            Stream = stream,
            Processor = new SimpleSelectProcessor<Correlated<TIn>, Correlated<TOut>>(i =>
            new Correlated<TOut> { Row = resultSelector(i.Row), CorrelationKeys = i.CorrelationKeys }),
            WithNoDispose = withNoDispose
        }).Output;
    public static IStream<Correlated<TOut>> Select<TIn, TKey, TOut>(this IStream<Correlated<TIn>> stream, string name, Func<TIn, TKey> sequenceGroupKeySelector, Func<TIn, int, TOut> resultSelector, bool withNoDispose = false) => new SelectStreamNode<Correlated<TIn>, Correlated<TOut>>(name, new SelectArgs<Correlated<TIn>, Correlated<TOut>>
    {
        Stream = stream,
        Processor = new SelectWithSequenceProcessor<Correlated<TIn>, TKey, Correlated<TOut>>((i, idx) =>
        new Correlated<TOut> { Row = resultSelector(i.Row, idx) }, i => sequenceGroupKeySelector(i.Row)),
        WithNoDispose = withNoDispose
    }).Output;
    public static ISingleStream<Correlated<TOut>> Select<TIn, TOut>(this ISingleStream<Correlated<TIn>> stream, string name, Func<TIn, TOut> resultSelector, bool withNoDispose = false) =>
        new SelectSingleStreamNode<Correlated<TIn>, Correlated<TOut>>(name, new SelectSingleArgs<Correlated<TIn>, Correlated<TOut>>
        {
            Stream = stream,
    ...
    public static IStream<Correlated<TOut>> Select<TIn, TOut>(this IStream<Correlated<TIn>> stream, string name, ISelectWithIndexProcessor<Correlated<TIn>, Correlated<TOut>> processor, bool withNoDispose = false) =>
        new SelectWithIndexStreamNode<Correlated<TIn>, Correlated<TOut>>(name, new SelectWithIndexArgs<Correlated<TIn>, Correlated<TOut>>
        {
            Stream = stream,
            Processor = processor,
            WithNoDispose = withNoDispose
    ...
    /// <typeparam name="TOut">Output type</typeparam>
    /// <returns>Output stream</returns>
    public static IStream<Correlated<TOut>> Select<TIn, TOut>(this IStream<Correlated<TIn>> stream, string name, Func<TIn, int, TOut> resultSelector, bool withNoDispose = false) => new SelectWithIndexStreamNode<Correlated<TIn>, Correlated<TOut>>(name, new SelectWithIndexArgs<Correlated<TIn>, Correlated<TOut>>
    {
        Stream = stream,
        Processor = new SimpleSelectWithIndexProcessor<Correlated<TIn>, Correlated<TOut>>((i, idx) =>
        new Correlated<TOut> { Row = resultSelector(i.Row, idx), CorrelationKeys = i.CorrelationKeys }),
        WithNoDispose = withNoDispose
    }).Output;
    /// <summary>
    /// Transform the input stream into a similar stream but with a different structure and calculation using a lambda expression. 
    ...
    public static IStream<Correlated<TOut>> Select<TIn1, TIn2, TOut>(this IStream<Correlated<TIn1>> stream, string name, ISingleStream<TIn2> streamToApply, Func<TIn1, TIn2, int, TOut> resultSelector, bool withNoDispose = false) =>
        new ApplyStreamNode<Correlated<TIn1>, TIn2, Correlated<TOut>>(name, new ApplyArgs<Correlated<TIn1>, TIn2, Correlated<TOut>>
        {
            MainStream = stream,
            StreamToApply = streamToApply,
    ...
    public static ISingleStream<Correlated<TOut>> Select<TIn1, TIn2, TOut>(this ISingleStream<Correlated<TIn1>> stream, string name, ISingleStream<TIn2> streamToApply, Func<TIn1, TIn2, int, TOut> resultSelector, bool withNoDispose = false) =>
        new ApplySingleStreamNode<Correlated<TIn1>, TIn2, Correlated<TOut>>(name, new ApplySingleArgs<Correlated<TIn1>, TIn2, Correlated<TOut>>
        {
            MainStream = stream,
            StreamToApply = streamToApply,
            IndexSelector = (i1, i2, idx) => new Correlated<TOut> { Row = resultSelector(i1.Row, i2, idx), CorrelationKeys = i1.CorrelationKeys },
    ...
    /// <typeparam name="TOut">Output type</typeparam>
    /// <returns>Output stream</returns>
    public static IStream<Correlated<TOut>> SelectCorrelated<TIn, TOut>(this IStream<Correlated<TIn>> stream, string name, Func<TIn, TOut> resultSelector, bool withNoDispose = false) =>
        new SelectStreamNode<Correlated<TIn>, Correlated<TOut>>(name, new SelectArgs<Correlated<TIn>, Correlated<TOut>>
        {
            Stream = stream,
            Processor = new SimpleSelectProcessor<Correlated<TIn>, Correlated<TOut>>(i => new Correlated<TOut> { Row = resultSelector(i.Row), CorrelationKeys = i.CorrelationKeys }),
    ...
    public static IStream<Correlated<TOut>> SelectCorrelated<TIn, TKey, TOut>(this IStream<Correlated<TIn>> stream, string name, Func<TIn, TKey> sequenceGroupKeySelector, Func<TIn, int, TOut> resultSelector, bool withNoDispose = false) =>
        new SelectStreamNode<Correlated<TIn>, Correlated<TOut>>(name, new SelectArgs<Correlated<TIn>, Correlated<TOut>>
        {
            Stream = stream,
    ...
    public static ISingleStream<Correlated<TOut>> SelectCorrelated<TIn, TOut>(this ISingleStream<Correlated<TIn>> stream, string name, Func<TIn, TOut> resultSelector, bool withNoDispose = false) =>
        new SelectSingleStreamNode<Correlated<TIn>, Correlated<TOut>>(name, new SelectSingleArgs<Correlated<TIn>, Correlated<TOut>>
        {
            Stream = stream,
            Processor = new SimpleSelectProcessor<Correlated<TIn>, Correlated<TOut>>(i => new Correlated<TOut> { Row = resultSelector(i.Row), CorrelationKeys = i.CorrelationKeys }),
    ...
    /// <typeparam name="TOut">Output type</typeparam>
    /// <returns>Output stream</returns>
    public static IStream<Correlated<TOut>> SelectCorrelated<TIn, TOut>(this IStream<Correlated<TIn>> stream, string name, ISelectProcessor<Correlated<TIn>, Correlated<TOut>> processor, bool withNoDispose = false) =>
        new SelectStreamNode<Correlated<TIn>, Correlated<TOut>>(name, new SelectArgs<Correlated<TIn>, Correlated<TOut>>
        {
            Stream = stream,
            Processor = processor,
    ...
    public static ISingleStream<Correlated<TOut>> SelectCorrelated<TIn, TOut>(this ISingleStream<Correlated<TIn>> stream, string name, ISelectProcessor<Correlated<TIn>, Correlated<TOut>> processor, bool withNoDispose = false) =>
        new SelectSingleStreamNode<Correlated<TIn>, Correlated<TOut>>(name, new SelectSingleArgs<Correlated<TIn>, Correlated<TOut>>
        {
            Stream = stream,
            Processor = processor,
    ...
    /// <typeparam name="TOut">Output type</typeparam>
    /// <returns>Output stream</returns>
    public static IStream<Correlated<TOut>> SelectCorrelated<TIn, TOut>(this IStream<Correlated<TIn>> stream, string name, ISelectWithIndexProcessor<Correlated<TIn>, Correlated<TOut>> processor, bool withNoDispose = false) =>
        new SelectWithIndexStreamNode<Correlated<TIn>, Correlated<TOut>>(name, new SelectWithIndexArgs<Correlated<TIn>, Correlated<TOut>>
        {
            Stream = stream,
            Processor = processor,
    ...
    /// <typeparam name="TOut">Output type</typeparam>
    /// <returns>Output stream</returns>
    public static IStream<Correlated<TOut>> SelectCorrelated<TIn, TOut>(this IStream<Correlated<TIn>> stream, string name, Func<TIn, int, TOut> resultSelector, bool withNoDispose = false) =>
        new SelectWithIndexStreamNode<Correlated<TIn>, Correlated<TOut>>(name, new SelectWithIndexArgs<Correlated<TIn>, Correlated<TOut>>
        {
            Stream = stream,
    ...
    /// <typeparam name="TOut">Output stream</typeparam>
    /// <returns>Output stream</returns>
    public static IStream<Correlated<TOut>> SelectCorrelated<TIn1, TIn2, TOut>(this IStream<Correlated<TIn1>> stream, string name, ISingleStream<TIn2> streamToApply, Func<TIn1, TIn2, int, TOut> resultSelector, bool withNoDispose = false) =>
        new ApplyStreamNode<Correlated<TIn1>, TIn2, Correlated<TOut>>(name, new ApplyArgs<Correlated<TIn1>, TIn2, Correlated<TOut>>
        {
            MainStream = stream,
            StreamToApply = streamToApply,
    ...
    public static ISingleStream<Correlated<TOut>> SelectCorrelated<TIn1, TIn2, TOut>(this ISingleStream<Correlated<TIn1>> stream, string name, ISingleStream<TIn2> streamToApply, Func<TIn1, TIn2, int, TOut> resultSelector, bool withNoDispose = false) =>
        new ApplySingleStreamNode<Correlated<TIn1>, TIn2, Correlated<TOut>>(name, new ApplySingleArgs<Correlated<TIn1>, TIn2, Correlated<TOut>>
        {
            MainStream = stream,
            StreamToApply = streamToApply,
    ...
    /// <typeparam name="TOut">Output stream</typeparam>
    /// <returns>Output stream</returns>
    public static IStream<Correlated<TOut>> SelectCorrelated<TIn1, TIn2, TOut>(this IStream<Correlated<TIn1>> stream, string name, ISingleStream<TIn2> streamToApply, Func<TIn1, TIn2, TOut> resultSelector, bool withNoDispose = false) =>
        new ApplyStreamNode<Correlated<TIn1>, TIn2, Correlated<TOut>>(name, new ApplyArgs<Correlated<TIn1>, TIn2, Correlated<TOut>>
        {
            MainStream = stream,
            StreamToApply = streamToApply,
    ...
    public static ISingleStream<Correlated<TOut>> SelectCorrelated<TIn1, TIn2, TOut>(this ISingleStream<Correlated<TIn1>> stream, string name, ISingleStream<TIn2> streamToApply, Func<TIn1, TIn2, TOut> resultSelector, bool withNoDispose = false) =>
        new ApplySingleStreamNode<Correlated<TIn1>, TIn2, Correlated<TOut>>(name, new ApplySingleArgs<Correlated<TIn1>, TIn2, Correlated<TOut>>
        {
            MainStream = stream,
            StreamToApply = streamToApply,
