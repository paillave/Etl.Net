...
public class SelectStreamNode<TIn, TOut>(string name, SelectArgs<TIn, TOut> args) : StreamNodeBase<TOut, IStream<TOut>, SelectArgs<TIn, TOut>>(name, args)
{
    ...
    {
        var processRow=args.Processor.ProcessRow;
        IPushObservable<TOut> obs = args.Stream.Observable.Map(WrapSelectForDisposal<TIn, TOut>(processRow, args.WithNoDispose));
